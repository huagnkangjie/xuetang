 ...
 * 例外保存
 * @param inputString
 * @param exception 例外标签
 * @return
 */
	
	public static String removeHtmlTagException(String inputString,String exception,String replace) {  
	    ...
	    java.util.regex.Pattern p_html;  
	    java.util.regex.Matcher m_html;  
	    //<[/]*?(?!br|/a|table|/table|tr|/tr|td|/td)[^<>]*?>
	    String regEx_html = "<[/]*?(?!"+exception+"/)[^<>]*?>"; // 定义HTML标签的正则表达式  
	    p_html = Pattern.compile(regEx_html, Pattern.CASE_INSENSITIVE);  
        m_html = p_html.matcher(htmlStr);  
        htmlStr = m_html.replaceAll("");// 过滤html标签  
        htmlStr=htmlStr.replace("<"+exception+"/>",  replace).replace("<"+exception.toUpperCase()+"/>",replace);
        textStr = htmlStr;  
	    ...
	    return  replaceHtmlTagSymbol(textStr) ;
	}
	
	
	public static String  replaceHtmlTagSymbol(String str){
	...
	public static void main(String args[]) {
		...
		System.out.println(removeHtmlTagException("<p>1、负责软件测试团队管理工作；<BR/>2、负责各类网站的性能/自动化测试工作；<br/>3、负责带领测试团队，设计、执行、优化测试过程,丰富测试手段，引入新的测试框架和测试策略；<br/>4、与其他测试人员、开发人员、项目管理人员沟通和协作，推动整个项目的顺利进行；<br/>5、&nbsp;&nbsp;&nbsp;维护测试流程，统计和分析测试结果，提高测试效率和质量。<br/><br>6.faf</br></p>","br","\n"));
