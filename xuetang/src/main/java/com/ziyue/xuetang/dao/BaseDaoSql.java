...
public class BaseDaoSql extends SqlSessionDaoSupport {
	private Logger log4j = Logger.getLogger(BaseDaoSql.class);
...
@Resource
	...
	public void batchUpdate(final String statementName, final List list) throws DataAccessException {
		...
		try {
			...
			if (log4j.isDebugEnabled()) {
				e.printStackTrace();
				log4j.debug("batchUpdate error: id [" + statementName + "], parameterObject [" + list + "].  Cause: "
						+ e.getMessage());
			}
		} finally {
			session.close();
	...
	public void batchInsert(final String statementName, final List list) throws DataAccessException {
		...
		try {
			...
			if (log4j.isDebugEnabled()) {
				e.printStackTrace();
				log4j.debug("batchInsert error: id [" + statementName + "], parameterObject [" + list + "].  Cause: "
						+ e.getMessage());
			}
		} finally {
			session.close();
	...
	public void batchDelete(final String statementName, final List list) throws DataAccessException {
		...
		try {
			...
			if (log4j.isDebugEnabled()) {
				e.printStackTrace();
				log4j.debug("batchDelete error: id [" + statementName + "], parameterObject [" + list + "].  Cause: "
						+ e.getMessage());
			}
		} finally {
			session.close();
